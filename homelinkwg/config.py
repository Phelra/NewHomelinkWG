...
def load_auth_config() -> None:
    ...
    flog("INFO", "config", f"Auth config loaded: password_hash={'set' if ADMIN_PASSWORD_HASH else 'NOT SET'}")


def get_admin_password_hash() -> str | None:
    """Get the current admin password hash (reads from module-level global)."""
